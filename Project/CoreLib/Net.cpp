...
int Net::SendPacket(Packet& packet)
{
    ...
    do
    {
        ...
        {
            ::MessageBoxA(g_hWindowHandle, "클라이언트 송신 오류", "경고", 0);
            return -1;
        }
        sendSize += recvBytes;
...
int Net::RecvPacket()
{
    ...
    if (recvBytes == 0) // 연결종료
    {
        ::MessageBoxA(g_hWindowHandle, "서버와 연결이 끊김.", "공지", 0);
        ::closesocket(m_clientSock);
        return 0;
    }
    if (recvBytes == SOCKET_ERROR)
...
int Net::RecvProcess(int recvBytes)
{
